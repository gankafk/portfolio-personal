...
(function () {
  ...
  var I18N = {
    ...
    "projects.type.container": { en: "Containers · DevOps", es: "Contenedores · DevOps" },
    "projects.docker.status": { en: "UNDER CONSTRUCTION", es: "EN CONSTRUCCIÓN" },
    "projects.docker.desc": { en: "A containers lab that grows from a single service to a multi-service system orchestrated on Kubernetes — with CI/CD, observability and image security.", es: "Laboratorio de contenedores que evoluciona de un servicio único a un sistema multiservicio orquestado en Kubernetes, con CI/CD, observabilidad y seguridad de imágenes." },
    "dock2.label": { en: "case study · containers", es: "caso de estudio · contenedores" },
    "dock2.lead": { en: "A hands-on containers lab, built step by step: a Python/Flask API with PostgreSQL and Redis that starts as a single Docker Compose service and grows into a hardened, multi-service system with CI/CD, observability and a final migration to Kubernetes. The goal is to justify every decision and show the full path from containers to orchestration.", es: "Un laboratorio práctico de contenedores, construido paso a paso: una API en Python/Flask con PostgreSQL y Redis que arranca como un único servicio en Docker Compose y crece hasta un sistema multiservicio endurecido, con CI/CD, observabilidad y una migración final a Kubernetes. El objetivo es justificar cada decisión y demostrar el recorrido completo de contenedores a orquestación." },
    "dock2.spec.pattern.v": { en: "Containers · multi-service", es: "Contenedores · multiservicio" },
    "dock2.spec.state": { en: "Status", es: "Estado" },
    "dock2.spec.state.v": { en: "Under construction", es: "En construcción" },
    "dock2.arch.title": { en: "Target architecture", es: "Arquitectura objetivo" },
    "dock2.arch.caption": { en: "The target architecture. A developer pushes code; GitHub Actions builds, scans and publishes images to the registry, which are deployed into the runtime. Orchestrated with Docker Compose today and migrating to a local Kubernetes cluster, the Flask API queries PostgreSQL, caches in Redis and offloads async jobs to a worker via a queue. Prometheus and Grafana collect metrics.", es: "La arquitectura objetivo. Un desarrollador sube código; GitHub Actions construye, escanea y publica imágenes en el registro, que se despliegan en el runtime. Orquestado hoy con Docker Compose y migrando a un clúster de Kubernetes local, la API Flask consulta PostgreSQL, cachea en Redis y delega trabajos asíncronos a un worker mediante una cola. Prometheus y Grafana recogen métricas." },
    "dock2.ov.title": { en: "What it is &amp; why I'm building it", es: "Qué es y por qué lo construyo" },
    "dock2.ov.p1": { en: "Docker Lab is a progressive project: I build a small but realistic containerised system and evolve it stage by stage, so I can understand and defend every decision — from image layers to orchestration.", es: "Docker Lab es un proyecto progresivo: construyo un sistema en contenedores pequeño pero realista y lo hago evolucionar etapa a etapa, para entender y poder defender cada decisión — desde las capas de imagen hasta la orquestación." },
    "dock2.ov.p2": { en: "It starts as a single Docker Compose service (Flask API + PostgreSQL + Redis) and grows toward a hardened, observable, multi-service system that finally runs on a local Kubernetes cluster.", es: "Arranca como un único servicio en Docker Compose (API Flask + PostgreSQL + Redis) y avanza hacia un sistema multiservicio, endurecido y observable, que finalmente corre en un clúster de Kubernetes local." },
    "dock2.ov.l1": { en: "<strong>Container-first</strong> — a Flask API with PostgreSQL and Redis, defined and run with Docker Compose.", es: "<strong>Contenedores desde el inicio</strong> — una API Flask con PostgreSQL y Redis, definida y ejecutada con Docker Compose." },
    "dock2.ov.l2": { en: "<strong>Hardened images</strong> — multi-stage builds, non-root users and vulnerability scanning in the pipeline.", es: "<strong>Imágenes endurecidas</strong> — builds multi-stage, usuarios no-root y escaneo de vulnerabilidades en el pipeline." },
    "dock2.ov.l3": { en: "<strong>Multi-service</strong> — the app splits into API + worker + queue for asynchronous processing.", es: "<strong>Multiservicio</strong> — la app se divide en API + worker + cola para procesamiento asíncrono." },
    "dock2.ov.l4": { en: "<strong>CI/CD</strong> — GitHub Actions builds, scans and publishes images to a registry (GHCR).", es: "<strong>CI/CD</strong> — GitHub Actions construye, escanea y publica imágenes en un registro (GHCR)." },
    "dock2.ov.l5": { en: "<strong>Observability &amp; orchestration</strong> — Prometheus + Grafana metrics, then a final migration to Kubernetes.", es: "<strong>Observabilidad y orquestación</strong> — métricas con Prometheus + Grafana y migración final a Kubernetes." },
    "dock2.dec.1.q": { en: "// why containers?", es: "// ¿por qué contenedores?" },
    "dock2.dec.1.h": { en: "Reproducible everywhere", es: "Reproducible en todas partes" },
    "dock2.dec.1.b": { en: "Compose describes the whole stack as code: the same environment on my laptop, in CI and in production. No «works on my machine».", es: "Compose describe todo el stack como código: el mismo entorno en mi portátil, en CI y en producción. Sin «en mi máquina funciona»." },
    "dock2.dec.2.q": { en: "// why harden images?", es: "// ¿por qué endurecer imágenes?" },
    "dock2.dec.2.h": { en: "Small, safe images", es: "Imágenes pequeñas y seguras" },
    "dock2.dec.2.b": { en: "Multi-stage builds keep images small; non-root users and vulnerability scans reduce the attack surface before anything ships.", es: "Los builds multi-stage mantienen las imágenes pequeñas; los usuarios no-root y el escaneo de vulnerabilidades reducen la superficie de ataque antes de publicar nada." },
    "dock2.dec.3.q": { en: "// why a worker + queue?", es: "// ¿por qué worker + cola?" },
    "dock2.dec.3.h": { en: "Async by design", es: "Asíncrono por diseño" },
    "dock2.dec.3.b": { en: "Splitting slow work into a worker behind a queue keeps the API responsive and lets each part scale independently.", es: "Separar el trabajo lento en un worker detrás de una cola mantiene la API ágil y permite escalar cada parte de forma independiente." },
    "dock2.dec.4.q": { en: "// why Kubernetes?", es: "// ¿por qué Kubernetes?" },
    "dock2.dec.4.h": { en: "From Compose to orchestration", es: "De Compose a la orquestación" },
    "dock2.dec.4.b": { en: "Compose is perfect to build and learn; migrating to a local Kubernetes cluster shows the real path to self-healing, scaling and rollouts.", es: "Compose es perfecto para construir y aprender; migrar a un clúster de Kubernetes local muestra el camino real hacia el autosanado, el escalado y los despliegues." },
    "dock2.road.title": { en: "The build plan, stage by stage", es: "El plan de construcción, etapa a etapa" },
    "dock2.road.intro": { en: "This lab is under active construction. The foundation runs; the rest is a clear, honest roadmap from containers to orchestration.", es: "Este lab está en construcción activa. La base ya funciona; el resto es una hoja de ruta clara y honesta, de los contenedores a la orquestación." },
    "dock2.road.done": { en: "// foundation · running", es: "// base · funcionando" },
    "dock2.road.next": { en: "// in progress · next", es: "// en curso · siguiente" },
    "dock2.m0": { en: "Base stack: Flask API + PostgreSQL + Redis on Docker Compose", es: "Stack base: API Flask + PostgreSQL + Redis en Docker Compose" },
    "dock2.m1": { en: "Image hardening: multi-stage, non-root, vulnerability scanning", es: "Endurecimiento de imágenes: multi-stage, no-root, escaneo de vulnerabilidades" },
    "dock2.m2": { en: "Split into services: API + worker + queue", es: "División en servicios: API + worker + cola" },
    "dock2.m3": { en: "CI/CD: build, scan and publish images (GitHub Actions &rarr; GHCR)", es: "CI/CD: construir, escanear y publicar imágenes (GitHub Actions &rarr; GHCR)" },
    "dock2.m4": { en: "Observability: Prometheus + Grafana", es: "Observabilidad: Prometheus + Grafana" },
    "dock2.m5": { en: "Migration to a local Kubernetes cluster", es: "Migración a un clúster de Kubernetes local" },
    "dock2.l.orch": { en: "Orchestration", es: "Orquestación" },
    "dock2.r.orch": { en: "Defines and runs the whole multi-container stack as code.", es: "Define y ejecuta todo el stack multicontenedor como código." },
    "dock2.l.api": { en: "API", es: "API" },
    "dock2.r.api": { en: "REST API and application logic.", es: "API REST y lógica de la aplicación." },
    "dock2.r.db": { en: "Relational persistence for the app.", es: "Persistencia relacional de la aplicación." },
    "dock2.l.cache": { en: "Cache · Broker", es: "Caché · Broker" },
    "dock2.r.cache": { en: "In-memory cache and job-queue backend.", es: "Caché en memoria y backend de la cola de trabajos." },
    "dock2.l.worker": { en: "Worker", es: "Worker" },
    "dock2.r.worker": { en: "Consumes queued jobs for async processing.", es: "Consume trabajos de la cola para procesamiento asíncrono." },
    "dock2.l.cicd": { en: "CI/CD", es: "CI/CD" },
    "dock2.r.cicd": { en: "Builds, scans and publishes images to GHCR.", es: "Construye, escanea y publica imágenes en GHCR." },
    "dock2.r.sec": { en: "Image hardening: non-root, minimal layers, vuln scanning.", es: "Endurecimiento de imágenes: no-root, capas mínimas, escaneo de vulnerabilidades." },
    "dock2.r.mon": { en: "Metrics, dashboards and alerts.", es: "Métricas, dashboards y alertas." },
    "dock2.l.k8s": { en: "Orchestration", es: "Orquestación" },
    "dock2.r.k8s": { en: "Target platform: self-healing, scaling and rollouts.", es: "Plataforma objetivo: autosanado, escalado y despliegues." },
    "dock2.r.ver": { en: "Everything versioned and documented, stage by stage.", es: "Todo versionado y documentado, etapa a etapa." },
    "dock2.cta.title": { en: "Follow the build", es: "Sigue la construcción" },
    "dock2.cta.sub": { en: "The repo grows stage by stage, each with its decisions and how to reproduce it — or get in touch to talk it through.", es: "El repo crece etapa a etapa, cada una con sus decisiones y cómo reproducirla — o escríbeme y lo comentamos." },
    "projects.lab.title": { en: "Linux + AWS Home Lab", es: "Linux + AWS Home Lab" },
    "projects.lab.desc": { en: "A self-hosted, secure hybrid infrastructure: 4 Ubuntu Server VMs (bastion, web, database and monitoring) on an isolated internal network, integrated with AWS and exposed safely through Cloudflare. Bastion-only SSH with hardening, an Nginx → Gunicorn → Flask → PostgreSQL stack, tested S3 backups, Prometheus + Grafana monitoring and a GitHub Actions CI/CD pipeline.", es: "Una infraestructura híbrida segura y self-hosted: 4 VMs Ubuntu Server (bastión, web, base de datos y monitorización) en red interna aislada, integrada con AWS y expuesta de forma segura con Cloudflare. Acceso SSH solo por bastión con hardening, stack Nginx → Gunicorn → Flask → PostgreSQL, backups verificados a S3, monitorización con Prometheus + Grafana y un pipeline CI/CD con GitHub Actions." },
    ...
    "lab2.lead": { en: "A self-hosted, secure hybrid infrastructure on four virtualized Ubuntu servers — bastion, web, database and monitoring — on an isolated internal network, integrated with AWS and exposed through Cloudflare. Built to prove, and defend in an interview, practical skills across Linux administration, networking, security, automation, monitoring, backups, cloud integration and CI/CD.", es: "Una infraestructura híbrida segura y self-hosted sobre cuatro servidores Ubuntu virtualizados —bastión, web, base de datos y monitorización— en red interna aislada, integrada con AWS y expuesta con Cloudflare. Construida para demostrar, y poder defender en una entrevista, competencias prácticas de administración Linux, redes, seguridad, automatización, monitorización, backups, integración cloud y CI/CD." },
    ...
    "lab2.arch.caption": { en: "Four Ubuntu VMs on VirtualBox in a segmented private LAN (10.10.10.0/24): the bastion is the only SSH entry point (ProxyJump), the web tier reaches the database only over the segmented link, a monitor node collects metrics, backups go offsite to AWS S3, and the service is exposed with no inbound ports via Cloudflare Tunnel.", es: "Cuatro VMs Ubuntu en VirtualBox dentro de una LAN privada segmentada (10.10.10.0/24): el bastión es el único punto de entrada SSH (ProxyJump), la capa web solo alcanza la base de datos por el enlace segmentado, un nodo de monitorización recoge métricas, los backups van offsite a AWS S3 y el servicio se expone sin puertos entrantes mediante Cloudflare Tunnel." },
    ...
    "lab2.road.intro": { en: "The lab was built incrementally, module by module — from Linux fundamentals to cloud integration and CI/CD — each one versioned and documented.", es: "El lab se construyó de forma incremental, módulo a módulo —de los fundamentos de Linux a la integración cloud y el CI/CD—, cada uno versionado y documentado." },
    "lab2.road.built": { en: "// built · modules 0–5", es: "// construido · módulos 0–5" },
    "lab2.road.planned": { en: "// built · modules 6–11", es: "// construido · módulos 6–11" },
    ...
    "lab2.l.cicd": { en: "CI/CD", es: "CI/CD" },
    "lab2.r.cicd": { en: "Self-hosted runner: automated deploys on push.", es: "Runner self-hosted: despliegues automáticos al hacer push." },
    ...
    "lab2.r.mon": { en: "Metrics, dashboards and alerts.", es: "Métricas, dashboards y alertas." },
    "lab2.r.cloud": { en: "Least-privilege identity and offsite backups.", es: "Identidad de mínimo privilegio y backups offsite." },
    "lab2.r.edge": { en: "Tunnel, WAF and HTTPS exposure with no inbound ports.", es: "Exposición con Tunnel, WAF y HTTPS sin puertos entrantes." },
